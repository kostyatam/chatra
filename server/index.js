...
app.get('/api/getMultiplayerGames', (req, res, next) => {
  ...
  Promise.all(requests).then(result => {
    const games = intersectionWith.apply(null, result.map(({games}) => games).concat((a, b) => a.appid === b.appid));
    return getAllSteamMultiplayerGames().then(multiplayerGames => {
      return games.filter((game) => multiplayerGames[game.appid]);
    });
  })
...
const getAllSteamMultiplayerGames = function () {
